"""watsonx LLM client for agent decision-making.
...
from app.schemas.simulation_models import AgentDecision, AgentSituation
...
async def generate_decision(
    ...
    async with _semaphore:
        # Re-check after acquiring semaphore — another agent may have tripped the breaker
        if _watsonx_disabled:
            return _rule_based_decision(situation)
        try:
            return await asyncio.wait_for(
                _call_watsonx(situation, model_id), timeout=5.0
            )
        except Exception:
